...
**Per-prompt model routing for [Pi](https://github.com/earendil-works/pi-coding-agent).**

While the router is on it owns the active model: every prompt is re-classified and the model is
re-decided before the turn runs, so a manual `/model` choice lasts only until your next prompt.
`/task-router off` opts out.

Classify the prompt → pick a model tier → `pi.setModel()` → tell you where it landed.

Package `pi-jev-task-router`. Install with `pi install git:github.com/rizafahmi/pi-jev-task-router@v0.1.2`
...
![routing per-prompt](https://img.shields.io/badge/routing-per--prompt-0EA5E9?style=flat-square)
...
| **Fallback** | Keyword heuristic: the offline path *and* the degraded path, always announced |
...
| **Tests** | `node --test` · 32 tests · no key, no network, and no install needed to run them |
...
> ### First run: the shipped tier models are one machine's catalogue
>
...
| the key is set but rejected (HTTP 401/403) | Routing turns itself off, the same as `/task-router off`: `router: jev rejected the API key (HTTP 403) - fix TYPESAFE_API_KEY, then /task-router on to re-enable`. The footer shows `off (jev key rejected)`. No heuristic fallback: the key is set, so Jev is what you asked for |
...
The rejected-key case is the one that changes state. The key is read once at load, so a key the
API refuses will not start working mid-session. Rather than pay a round trip and print a warning
on every prompt (or quietly route on keywords when you configured a classifier), the router stops
and tells you how to resume. The reason is written into the marker file, so a `/reload` or a later
session still explains why it is off instead of showing a bare `disabled`.
...
**model id** is what `pi` can actually load. `policy.ts` holds the default mapping. It is an
example taken from one model catalogue, not a recommendation, so point it at models you actually
have before trusting a route:
...
Override the defaults without editing code by setting `TASK_ROUTER_TIERS`, a JSON object,
partially or fully specified; omitted tiers keep their default:
...
The rotation here is two models, with `frontier` mapping to the same model as `balanced`.
"security → at least frontier" is what guarantees a security prompt never lands on the flash
model, and the tier distinction still drives the
confirm gate and the footer, even when the model behind two tiers is the same.

Cost order follows the tier table, so overriding the tiers overrides the cost order too; there
is no second table to keep in sync. That matters most when one model is served by
...
`policy.ts`. Both are single, reviewable places (TypeSafe's own advice).
...
`providers/fake.ts` is keyword matching, first match wins: security, tests, docs, refactor,
bugfix; then explicit vagueness; then "short prompt, no signal"; then a feature fallback. It is
biased toward over-triggering `security`, the safe direction.

It is the offline path and the fallback when Jev is unreachable.
...
`BASE_TIER` is the single kind → tier table:
...
The router also turns itself off when Jev rejects the API key, writing the same marker with the
reason as its content, so `/reload` and later sessions still say why. That is the only
...
no toolchain, no `npm install`). 32 tests, 16 each in `policy.test.ts` and
...
right-hand column assumes the default tier table; with `TASK_ROUTER_TIERS` set, expect your
own ids in the same tiers:
...
- "Add a login page to the admin dashboard" reads as security 0.71 / feature 0.29,
  so the security-first policy sends it to frontier. Over-triggering security is the
...
- `/router-check --jev` scores 16/17 with `jev-1.13.0` on the machine these fixtures were
  ...
  and the confidence floor lifting it to balanced. The floor is doing its job. The keyword
  fallback calls it bugfix/fast_cheap.
...
- The router owns the model while it is on, so a manual `/model` choice survives only until
  your next prompt; the router re-routes it. `/task-router off` is the way to keep a manual
  choice.
...
| on/off marker | `~/.pi/agent/pi-jev-task-router.disabled`, kept out of the package dir (see [On/off toggle](#onoff-toggle)) |
