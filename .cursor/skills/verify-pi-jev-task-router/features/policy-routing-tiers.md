...
- **ask_human veto (unclear).** Send a vague prompt. Inside Pi TUI, type `do it` and press Enter. Expect notify shows `router: not routing (unclear/S ask_human, no signal) - clarify the task or pick a model with /model` (warning level). No model switch. Footer unchanged from previous prompt. `/router` confirms outcome `ask_human`.
...
- Policy rules apply in order (see `applyPolicy()` in policy.ts): security escalation first, complexity bump, then ask_human veto, then confidence floors. Later rules can override earlier ones, except frontier is never downgraded.
- The keyword heuristic (fake provider) is deliberately biased toward security: it matches "XSS", "IDOR", "SQL injection", "login" (in certain contexts) as security. Over-triggering security is safer than under-triggering it.
