...
- `node --experimental-strip-types --test` runs `.ts` files directly from Node 22.19+ (package.json engines requirement), no build step, but `npm run typecheck` needs `npm install` for devDependencies. The control script handles this automatically.
